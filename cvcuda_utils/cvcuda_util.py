...
def create_image_batch(
    ...
    for i in range(num_images):
        ...
        print("type(create_image((w, h), img_format, max_random, rng)):", 
              type(create_image((w, h), img_format, max_random, rng)))
    return image_batch


def clone_image_batch(input_image_batch, newsize, img_format=None):
    ...
    for input_image in input_image_batch:
        # input_image.size = (1024,1024)
        image = nvcv.Image(
            newsize, input_image.format if img_format is None else img_format
        )
