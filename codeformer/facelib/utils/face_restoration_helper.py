...
class FaceRestoreHelper(object):
    ...
    def read_image(self, img):
        ...
        self.input_img = img
        self.is_gray = False    #is_gray(img, threshold=10)
        if self.is_gray:
    ...
    def init_dlib(self, detection_path, landmark5_path):
        ...
        detection_path = load_file_from_url(url=detection_path, model_dir='checkpoint/', progress=True, file_name=None)
        landmark5_path = load_file_from_url(url=landmark5_path, model_dir='checkpoint/', progress=True, file_name=None)
    ...
    def get_face_landmarks_5(self,
        ...
        if bboxes is None or bboxes.shape[0] == 0:
            log.logger.error('No face detected. Check your video.')
            return 0
        else:
            bboxes = bboxes / scale
    ...
    def align_warp_face(self, save_cropped_path=None, border_mode='constant'):
        ...
        for idx, landmark in enumerate(self.all_landmarks_5):
            ...
            # print( "for idx, landmark in enumerate(self.all_landmarks_5):" ,"跑了几次",idx)
    ...
    def get_inverse_affine(self, save_inverse_affine_path=None):
        ...
        # print("self.affine_matrices:",self.affine_matrices)
        
        for idx, affine_matrix in enumerate(self.affine_matrices):
            # print("get_inverse_affine:  跑第几次:   ", idx)
            
            inverse_affine = cv2.invertAffineTransform(affine_matrix)    # inverse_affine [2, 3]
    ...
    def paste_faces_to_input_image(self, face_parse_out, save_path=None, upsample_img=None, draw_box=False, face_upsampler=None):
        h, w, _ = self.input_img.shape
        ...
        # print("len(self.restored_faces)",len(self.restored_faces))
        # print("len(self.inverse_affine_matrices)",len(self.inverse_affine_matrices))
        
        # print("self.inverse_affine_matrices:", self.inverse_affine_matrices)
        ...
        inv_mask_borders = []
        for restored_face, inverse_affine in zip(self.restored_faces, self.inverse_affine_matrices):    # 每张脸都这么做
            ...
            inv_restored = cv2.warpAffine(restored_face, inverse_affine, (w_up, h_up))    # inv_restored 修复后的人脸贴回到黑色背景中
            ...
            if self.use_parse:
                ...
                parse_mask = np.zeros(face_parse_out.shape)
                MASK_COLORMAP = [0, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 0, 255, 0, 0, 0]
                for idx, color in enumerate(MASK_COLORMAP):
                    parse_mask[face_parse_out == idx] = color
                #  blur the mask
